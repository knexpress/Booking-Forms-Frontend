...
import { Camera, CheckCircle, XCircle, RotateCcw, ArrowLeft, AlertCircle } from 'lucide-react'
  ...
  const handleContinue = () => {
    if (frontImage && backImage) {
      onComplete({
        ...
        eidVerified: true,
        eidData: eidData, // Pass extracted data
      })
    } else {
      ...
      {frontImage && backImage && !error && (
        <div className="space-y-3">
          <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded-lg flex items-start gap-2">
            ...
            <div className="flex-1">
              <p className="text-green-700 font-semibold">Emirates ID scanned successfully!</p>
              {eidData && !eidData.captured && (
                <div className="mt-2 text-sm text-green-600">
                  <p>ID Number: {eidData.idNumber || 'Detected'}</p>
                  <p>Name: {eidData.name || 'Detected'}</p>
                  <p>Expiry: {eidData.expiryDate || 'Detected'}</p>
                </div>
              )}
              {eidData?.captured && (
                <div className="mt-2 text-sm text-green-600">
                  <p>✓ Front and back images captured</p>
                  <p>✓ Ready for identity verification</p>
                </div>
              )}
            </div>
          </div>
          
          <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg flex items-start gap-2">
            <AlertCircle className="w-5 h-5 text-blue-500 flex-shrink-0 mt-0.5" />
            <p className="text-blue-700 text-sm">
              {API_CONFIG.features.simulationMode 
                ? 'Your Emirates ID information has been verified and extracted. Please ensure all details are correct before proceeding.'
                : 'Emirates ID images captured. Identity verification will be performed with TRUE-ID in the next step.'}
            </p>
          </div>
        </div>
      )}
