...
export default function Step1BookingForm({ onComplete, initialData, service }: Step1Props) {
  ...
  // Dynamic labels/placeholders/patterns per route
  const senderHeader = `Sender Details ${isPhToUae ? '(Philippines)' : '(UAE)'}`
  const receiverHeader = `Receiver Details ${isPhToUae ? '(UAE)' : '(Philippines)'}`

  const senderPhonePattern = isPhToUae ? /^\+63[0-9]{10}$/ : /^\+971[0-9]{9}$/
  const senderPhonePlaceholder = isPhToUae ? '+63XXXXXXXXXX' : '+971XXXXXXXX'

  const receiverPhonePattern = isPhToUae ? /^\+971[0-9]{9}$/ : /^\+63[0-9]{10}$/
  const receiverPhonePlaceholder = isPhToUae ? '+971XXXXXXXX' : '+63XXXXXXXXXX'

  const warehouseLabel = isPhToUae ? 'UAE Warehouse Pick-Up' : 'PH Warehouse Pick-Up'
  const addressLabel = isPhToUae ? 'Deliver to UAE Address' : 'Deliver to PH Address'
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-8">
      ...
      <div className="space-y-4">
        <h3 className="section-header">{senderHeader}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            {...register('sender.contactNo', { 
                ...
                pattern: {
                  value: senderPhonePattern,
                  message: isPhToUae ? 'Enter valid PH number (+63XXXXXXXXXX)' : 'Enter valid UAE number (+971XXXXXXXXX)'
                }
              })}
              className="form-input"
              placeholder={senderPhonePlaceholder}
            />
      ...
      {/* Receiver Details */}
      <div className="space-y-4">
        <h3 className="section-header">{receiverHeader}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            {...register('receiver.contactNo', { 
                required: 'Contact number is required',
                pattern: {
                  value: receiverPhonePattern,
                  message: isPhToUae ? 'Enter valid UAE number (+971XXXXXXXXX)' : 'Enter valid PH number (+63XXXXXXXXXX)'
                }
              })}
              className="form-input"
              placeholder={receiverPhonePlaceholder}
            />
