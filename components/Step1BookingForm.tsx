...
export default function Step1BookingForm({ onNext, onBack, initialData, service }: Step1Props) {
  ...
  // Get available options based on route and selections
  ...
  const availableUaeCities = emirates ? getCitiesForEmirate(emirates) : []
  ...
  const onSubmit = (data: BookingFormData) => {
    ...
    const senderData: Partial<BookingFormData> = {
      sender: {
        ...
        lastName,
        completeAddress,
        country: isPhToUae ? 'PHILIPPINES' : 'UNITED ARAB EMIRATES',
        region: '',
        province: '',
        city: '',
        barangay: '',
