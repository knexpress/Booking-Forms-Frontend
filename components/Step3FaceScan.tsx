...
export default function Step3FaceScan({ onComplete, onBack, eidImage, eidBackImage, service }: Step3Props) {
  ...
  const performFinalVerification = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        console.log('📸 Step3 sending face data to onComplete:', {
          hasFaceImage: !!faceDataToSend.faceImage,
          faceImagesCount: faceDataToSend.faceImages?.length || 0,
          faceImageLength: faceDataToSend.faceImage?.length || 0
        })
        onComplete(faceDataToSend)
      }, 2000)
  ...
  const handleContinue = () => {
    console.log('📸 Step3 handleContinue called:', {
      ...
      success: success,
      faceImageLength: faceImage?.length || 0,
    ...
    if (faceImage && success) {
      const faceDataToSend = {
        faceImage: faceImage,
        faceImages: faceImages.length > 0 ? faceImages : [faceImage],
        faceVerified: true,
        livenessData: livenessResult,
        faceMatchData: verificationResult,
      }
      console.log('📸 Sending face data to onComplete:', {
        hasFaceImage: !!faceDataToSend.faceImage,
        faceImagesCount: faceDataToSend.faceImages?.length || 0
      })
      onComplete(faceDataToSend)
    } else {
      console.error('❌ Face scan not complete:', {
        hasFaceImage: !!faceImage,
        success: success
      })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-1 sm:space-y-2">
        ...
        <h3 className="text-base sm:text-lg lg:text-xl text-gray-600 font-medium">Face Scan</h3>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 p-3 sm:p-4 lg:p-5 rounded-lg shadow-sm">
        ...
        <ul className="text-xs sm:text-sm text-blue-800 space-y-1.5 sm:space-y-2">
          ...
          <li className="flex items-start gap-2">
            <span className="text-blue-600 font-bold mt-0.5 flex-shrink-0">•</span>
            <span>Follow the on-screen prompts for liveness detection</span>
          </li>
        </ul>
      </div>

      {/* Scanning Interface */}
      <div className="bg-gradient-to-br from-white to-gray-50 border-2 border-gray-200 rounded-xl p-3 sm:p-4 lg:p-6 shadow-sm">
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl aspect-video flex items-center justify-center border-2 border-dashed border-gray-300">
              <div className="text-center space-y-2 sm:space-y-3 px-4">
                <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 mx-auto bg-blue-100 rounded-full flex items-center justify-center">
                  <Eye className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-blue-600" />
                </div>
                <p className="text-xs sm:text-sm lg:text-base text-gray-600 font-medium">Ready to scan your face</p>
                <p className="text-[10px] sm:text-xs text-gray-500">Click below to start</p>
              </div>
            </div>
            ...
            <button
              type="button"
              onClick={startScan}
              className="btn-primary w-full flex items-center justify-center gap-2 text-xs sm:text-sm lg:text-base font-semibold py-2.5 sm:py-3 lg:py-4 shadow-lg hover:shadow-xl transition-all"
            >
              <Maximize2 className="w-4 h-4 sm:w-5 sm:h-5" />
              <Camera className="w-4 h-4 sm:w-5 sm:h-5" />
              <span className="hidden sm:inline">Start Face Scan (Full Screen)</span>
              <span className="sm:hidden">Start Face Scan</span>
            </button>
          </div>
        ) : faceImage ? (
      ...
      {/* Status Messages */}
      {error && (
        <div className="bg-red-50 border-l-4 border-red-500 p-3 sm:p-4 lg:p-5 rounded-lg shadow-sm">
          <div className="flex items-start gap-2 sm:gap-3">
            <XCircle className="w-4 h-4 sm:w-5 sm:h-5 text-red-500 flex-shrink-0 mt-0.5" />
            <div className="flex-1 min-w-0">
              <p className="text-red-700 font-medium text-xs sm:text-sm lg:text-base">{error}</p>
              {cameraError && (
      ...
      {success && (
        <div className="space-y-2 sm:space-y-3">
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-l-4 border-green-500 p-4 sm:p-6 lg:p-8 rounded-lg text-center shadow-sm">
            <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 mx-auto mb-3 sm:mb-4 bg-green-100 rounded-full flex items-center justify-center">
              <CheckCircle className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-green-500" />
            </div>
            <p className="text-green-700 font-bold text-lg sm:text-xl lg:text-2xl mb-1 sm:mb-2">
              Verification Successful!
            </p>
            <p className="text-green-600 text-xs sm:text-sm lg:text-base">
              Thank you for completing the face scan verification
            </p>
          </div>
        </div>
      )}
      ...
      {/* Navigation Buttons */}
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 lg:gap-4 pt-3 sm:pt-4 border-t border-gray-200">
        <button
          type="button"
          onClick={onBack}
          className="btn-secondary flex items-center justify-center gap-2 order-2 sm:order-1 text-xs sm:text-sm lg:text-base py-2 sm:py-2.5"
        >
        ...
        {!success && (
          <button
            ...
            className="btn-primary flex-1 flex items-center justify-center gap-2 order-1 sm:order-2 disabled:opacity-50 disabled:cursor-not-allowed text-xs sm:text-sm lg:text-base py-2 sm:py-2.5"
          >
            <span className="truncate">Proceed to Booking Confirmation</span>
            <svg className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
