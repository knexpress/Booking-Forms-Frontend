...
export function findDocumentContour(src: any): any[] | null {
  ...
  try {
    ...
    // Apply stronger Gaussian blur to reduce internal noise and focus on card edges
    const blurSize = isMobile ? 7 : 5;  // More blur to smooth internal details
    ...
    window.cv.morphologyEx(blur, morphed, window.cv.MORPH_CLOSE, kernel);
    kernel.delete();

    // Apply Canny edge detection - adjusted to focus on strong outer edges
    const cannyLow = isMobile ? 20 : 30;   // Higher to reduce internal edges
    const cannyHigh = isMobile ? 80 : 120; // Higher to focus on strong edges
    window.cv.Canny(morphed, edges, cannyLow, cannyHigh);
    morphed.delete();
    ...
    // Minimum area should be at least 5% of image (to prioritize card over internal shapes)
    const minAreaPercent = 0.05;
    const minArea = Math.max(isMobile ? 5000 : 8000, imgArea * minAreaPercent);

    let maxArea = 0;
    let largestContour: any = null;
    let bestScore = 0;

    // Find the largest contour that matches card characteristics
    for (let i = 0; i < contours.size(); i++) {
      ...
      if (area > minArea && area > maxArea) {
        ...
        // ID cards typically have aspect ratio around 1.5-1.7 (85.6mm x 53.98mm ≈ 1.586)
        // Allow range of 1.3 to 2.0 to be flexible
        const minAspectRatio = 1.3;
        const maxAspectRatio = 2.0;
        
        // Approximate contour to polygon
        const epsilonFactor = isMobile ? 0.03 : 0.02;  // Less lenient for cleaner edges
        const epsilon = epsilonFactor * window.cv.arcLength(contour, true);
        const approx = new window.cv.Mat();
        window.cv.approxPolyDP(contour, approx, epsilon, true);

        // Check if it's roughly rectangular (4-6 corners for cleaner detection)
        const minCorners = 4;
        const maxCorners = 6;  // Stricter to avoid internal shapes
        
        // Calculate score: prioritize larger area and correct aspect ratio
        let score = area;
        if (aspectRatio >= minAspectRatio && aspectRatio <= maxAspectRatio) {
          score *= 1.5; // Boost score for correct aspect ratio
        }
